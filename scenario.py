...
class Scenario:
    ...
    def display(self):
        if self.aux_time > 0:
            ...
            self.aux_beat -= 1
            if self.aux_beat <= 0:
                self.aux_beat = 100 - 6*self.aux_beat2

            if self.aux_beat == 100 - 6*self.aux_beat2 and not self.run_beat3:
            ...
            if self.aux_beat2 <= 10 and not pygame.mixer.Channel(6).get_busy() and self.aux_beat3 <= 0:
                self.aux_beat2 += 1
                self.aux_beat3 = 5
